...
public class InternalNioInputBuffer extends AbstractInputBuffer<NioChannel> {
    ...
    private static final org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory.getLog(InternalNioInputBuffer.class);
    ...
    public boolean parseRequestLine(boolean useAvailableDataOnly) throws IOException {
        ...
        if ( parsingRequestLinePhase == 0 ) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Received [" + new String(buf, pos, lastValid - pos, DEFAULT_CHARSET) + "]");
            }
        }
    ...
    private void expand(int newsize) {
        if ( newsize > buf.length ) {
            ...
            // Should not happen
            log.warn("Expanding buffer size. Old size: " + buf.length + ", new size: " + newsize, new Exception());
    ...
    private HeaderParseStatus skipLine() throws IOException {
        ...
        if (log.isDebugEnabled()) {
            log.debug(sm.getString("iib.invalidheader", new String(buf, headerData.start, headerData.lastSignificantChar - headerData.start + 1, DEFAULT_CHARSET)));
        }
    ...
    @Override
    protected void init(SocketWrapper<NioChannel> socketWrapper, AbstractEndpoint endpoint) throws IOException {

        socket = socketWrapper.getSocket();
        socketReadBufferSize = socket.getBufHandler().getReadBuffer().capacity();

        int bufLength = skipBlankLinesSize + headerBufferSize + socketReadBufferSize;
